...
var LoaderButtonMixin = {
    _prepareButton() {
        ...
        this.innerHTML = `
      <p class="loader-button__text"><svg focusable="false" width="17px" height="17px" class="{{ icon_class }}" viewBox="0 0 17 17">
...
var FlickityControls = class extends CustomHTMLElement {
    ...
    async _onSlideChanged(animate = true) {
        let flickityInstance = await this.flickityCarousel.flickityInstance,
            activeItems = Array.from(this.querySelectorAll(`[aria-controls="${flickityInstance.selectedElement.id}"]`));
...
function getMediaSrcset(media, sizeList) {
    let srcset = [],
        supportedSizes = typeof media === "string" ? sizeList : getSupportedSizes(media, sizeList);
...
var TestimonialItem = class extends CustomHTMLElement {
    ...
    async transitionToLeave(shouldAnimate = true) {
        const textLines = await resolveAsyncIterator(this.querySelectorAll("split-lines, .testimonial__author")),
            animation = new CustomAnimation(new ParallelEffect(textLines.reverse().map((item, index) => {
                return new CustomKeyframeEffect(item, {
    ...
    async transitionToEnter(shouldAnimate = true) {
        const textLines = await resolveAsyncIterator(this.querySelectorAll("split-lines, .testimonial__author")),
            animation = new CustomAnimation(new ParallelEffect(textLines.map((item, index) => {
                return new CustomKeyframeEffect(item, {
                    visibility: ["hidden", "visible"],
    ...
    async _buildItems() {
        ...
        return Promise.resolve(activeImages.map((item) => {
            const matchedMedia = product["media"].find((media) => media.id === parseInt(item.getAttribute("data-media-id"))),
                supportedSizes = getSupportedSizes(matchedMedia, [200, 300, 400, 500, 600, 700, 800, 1e3, 1200, 1400, 1600, 1800, 2e3, 2200, 2400, 2600, 2800, 3e3]),
